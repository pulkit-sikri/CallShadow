...
def extract_acoustic_features(waveform_16k: np.ndarray, sample_rate: int = 16000) -> Dict[str, float]:
    """
    High-speed vectorized acoustic feature extraction for explainability and heuristic detection:
    - pitch_std: standard deviation of pitch (F0 via fast autocorrelation)
    - pitch_mean: mean F0
    - spectral_centroid_std_ratio: ratio of spectral centroid std to mean
    - energy_flatness: mean spectral flatness
    - zero_crossing_rate: mean ZCR
    - spectral_flux: frame-to-frame spectral magnitude differences
    """
    features = {
        "pitch_std": 0.0,
        "pitch_mean": 0.0,
    ...
    if waveform_16k is None or len(waveform_16k) < sample_rate * 0.1:  # less than 100ms
        return features

    try:
        # Fast STFT via scipy.signal.stft (computed once for multiple spectral features)
        nperseg = 512
        noverlap = 256
        f, t, Zxx = scipy.signal.stft(waveform_16k, fs=sample_rate, nperseg=nperseg, noverlap=noverlap)
